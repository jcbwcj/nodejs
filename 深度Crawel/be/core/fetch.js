...
exports.fetchList = (function(config) {
	...
	function fetchListDetail(arr, $) {
		...
		fn.fetch($(arr[item - 1]).find(".position_link").attr("href"), function(data) {
			...
			//数据库操作
			if(isSave) {
				mysql.connection.query(`INSERT INTO lagou(position_id, company, salary, position_name, company_id, hrid, position_link, job_introduce, company_img, industry, description) VALUES (
